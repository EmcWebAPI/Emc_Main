...
namespace EmcReportWebApi.Controllers
{
    public class ReportController : ApiController
    {
        ...
        private string SetConductedEmission(WordUtil wordUtil, JObject jObject, string bookmark)
        {
            ...
            string sysjTemplateFilePath = GetTemplatePath("RTFTemplate.docx");
            ...
            RtfTableInfo rtfTableInfo = MyTools.RtfTableInfos.Where(p => p.RtfType=="CE").FirstOrDefault();
            ...
            RtfPictureInfo rtfPictureInfo = MyTools.RtfPictureInfos.Where(p => p.RtfType == "CE").FirstOrDefault();
            ...
            foreach (JObject item in sysj)
            {
                ...
                contentList.Add(item["sygdy"].ToString());
                contentList.Add(item["syplfw"].ToString());
                contentList.Add(item["ypyxms"].ToString());
                ...
                foreach (JObject rtfObj in (JArray)item["rtf"])
                {
                    ...
                    wordUtil.CopyOtherFilePictureToWord(sysjTemplateFilePath, GetRtfPath(rtfObj["name"].ToString()), imageStartIndex, imageBookmark, false,true);
                }
        ...
        private string InsertRtfIntoReport(string fileName, string htmlstr)
        {
            ...
            using (WordUtil wordUtil = new WordUtil(outfilePth, filePath))
            {
                ...
                wordUtil.CopyOtherFilePictureToWord(rtfFullName, startIndex, bookmark,false);
            }
