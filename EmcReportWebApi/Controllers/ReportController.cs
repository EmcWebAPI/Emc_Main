    ...
    public class ReportController : ApiController
    {
        ...
        public IHttpActionResult CreateStandardReport(ReportParams para)
        {
            ...
            try
            {
                ...
                ZipFileHelper.DecompressionZip(reportZipFilesPath, reportFilesPath);
                //生成报告
                string content = JsonToWordStandard(reportId.Equals("") ? "QW2018-698" : reportId, para.JsonStr.Equals("") ? jsonStr1 : para.JsonStr, reportFilesPath);
                sw.Stop();
                double time1 = (double)sw.ElapsedMilliseconds / 1000;
                result = SetReportResult<string>(string.Format("报告生成成功,用时:" + time1.ToString()), true, content);
                MyTools.InfoLog.Info("报告:" + result.Content + ",信息:" + result.Message);
        ...
        public string Get3()
        {
            ...
            string result = JsonToWordStandard("QT2019-3015", jsonStr, reportFilesPath);
            //string result = "";
            sw.Stop();
            double time1 = (double)sw.ElapsedMilliseconds / 1000;
            return result + ":" + time1.ToString();
        ...
        #region 生成标准报表方法
        private string JsonToWordStandard(string reportId, string jsonStr, string reportFilesPath)
        {
            //解析json字符串
            JObject mainObj = (JObject)JsonConvert.DeserializeObject(jsonStr);
            string currRoot = AppDomain.CurrentDomain.BaseDirectory;
            ...
            string outfilePth = string.Format(@"{0}\Files\OutPut\{1}", currRoot, outfileName);//输出文件路径
            string filePath = string.Format(@"{0}\Files\{1}", currRoot, ConfigurationManager.AppSettings["StandardTemplateName"].ToString());//模板文件
