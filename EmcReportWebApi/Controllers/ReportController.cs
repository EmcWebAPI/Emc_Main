...
namespace EmcReportWebApi.Controllers
{
    public class ReportController : ApiController
    {
            ...
            using (WordUtil wordUtil = new WordUtil(outfilePth, filePath))
            {
                ...
                foreach (JObject item in experiment)
                {
                    if (item["name"].ToString().Equals("传导发射实验"))
                        newBookmark = SetConductedEmission(wordUtil, item, newBookmark, "CE");
                    else if (item["name"].ToString().Equals("辐射发射试验"))
                        newBookmark = SetConductedEmission(wordUtil, item, newBookmark, "RE");
                }
            }
        ...
        /// </summary>
        /// <returns></returns>
        private string SetConductedEmission(WordUtil wordUtil, JObject jObject, string bookmark, string rtfType)
        {
