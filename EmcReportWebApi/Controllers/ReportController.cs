...
namespace EmcReportWebApi.Controllers
{
    public class ReportController : ApiController
    {
        ...
        public string Get3() {
            Stopwatch sw = new Stopwatch();
            sw.Start();
            string result = this.TestCreateReportDirectory("11");
            sw.Stop();
            double time1 = (double)sw.ElapsedMilliseconds / 1000;
            return result + ":" + time1.ToString();
        }
        ...
        private string JsonToWord(string jsonStr)
        {
            ...
            using (WordUtil wordUtil = new WordUtil(outfilePth, filePath))
            {
                ...
                //受检样品描述 object
                JObject sjypms = (JObject)mainObj["sjypms"];
                result = ExperimentSample(wordUtil, sjypms);
                if (!result.Equals("保存成功"))
                {
                    return result;
                }
                ...
                foreach (JObject item in experiment)
                {
                    ...
                    //else if (item["name"].ToString().Equals("静电放电")) {
                    //    newBookmark = SetConductedEmission(wordUtil, item, newBookmark, "RE", middleDir);
                    //}
                }
            }
        ...
        //实验样品描述
        private string ExperimentSample(WordUtil wordUtil, JObject jo1)
        {
            //复选框内容
            ...
            foreach (var item in jo1)
            {
                ...
                if (checkList.Contains(key))
                {
                    if (value.ToLower().Equals("true"))
                        wordUtil.SelecionCheckbox(key, 1);
                }
                else if (doubleCheckList.Contains(key))
                {
                    if (value.ToLower().Equals("false") || value.ToLower().Equals(""))
                        wordUtil.SelecionCheckbox(key, 2);
                    else
                        wordUtil.SelecionCheckbox(key, 1);
                }
                else
